...
async function DBCleanup() {
  try {
    ...
    if (sensorRecordsCount > MAX_DOCS) {
      ...
      console.log(`Count capping: deleted ${capRes.deletedCount} docs older than ${boundaryDoc.timestamp.toISOString()}`, 'CLEANUP');
    }
  } catch (err) {
...
function hourlyDBCleanup() {
  ...
  const runCleanup = async () => {
    if (isCleaning) {
      console.log("Cleanup already running, skipping this tick", 'CLEANUP');
      return;
    }
    isCleaning = true;
    try {
...
const server = net.createServer((socket) => {
  ...
  socket.on("data", async (data) => {
    ...
    try {
      ...
      console.log(`Raw data received (${data.length} bytes) from`, clientInfo);
      ...
      while (buffer.length >= 58) {
        ...
        console.log(`✅ Packet processed successfully for MAC: ${mac}`, `Time: ${getFormattedDateTime()}`);
      }
    } catch (err) {
