...
/**
 * Captures an image from a camera using an external executable (ReadImage).
 *
 * @param {string} ip - IP address of the camera
 * @param {string} outputPath - Path where the captured image will be saved
 *
 * Workflow:
 * 1. Resolve executable path (platform aware: Windows/Linux)
 * 2. Build arguments for executable
 * 3. Spawn child process to run executable
 * 4. Handle:
 *    - Runtime errors
 *    - Timeout
 *    - Exit status
 * 5. Validate output file (existence + size)
 */
async function captureTechno(ip, outputPath) {
    // const exePath = process.env.READIMAGE_EXE_PATH || path.join(__dirname, "ReadImage.exe");
    
    // Step 1: Resolve executable path based on OS
    console.log(process.platform);
    const exePath = process.env.READIMAGE_EXE_PATH || path.join(
    __dirname,
    process.platform === "win32" ? "ReadImage.exe" : "ReadImage"
    );

    // Step 2: Configure timeout (default: 20 seconds)
    ...
    if (process.env.READIMAGE_ARGS_JSON) {
        try {
            ...
            args = parsed.map((a) => String(a).replaceAll("{ip}", String(ip)).replaceAll("{out}", String(outputPath)));
        } catch (e) {
            throw new Error(`Invalid READIMAGE_ARGS_JSON: ${e.message}`);
        }
    }

    // Step 4: Execute external process
